    ...
    while [ $# -gt 0 ]; do
        ...
        if [ -f "$filename" ]; then             # if exists
            if  [ $sourcetype == "mp4" ] ||
                ...
                datetime=$(echo $datetime | grep -Eo "([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2})") # filter out unreasonable 'UTC'-marker
                ...
                # black circle
#                s_dialplate="convert -size "$dialsize"x"$dialsize" xc:white -fill $transparency -stroke black "                        # white background
                s_dialplate="convert -size "$dialsize"x"$dialsize" xc:transparent -fill $transparency -stroke black "                   # transparent background
                ...
                echo "now generating a lot of images with dials"
                startsecs=$(date -d "$datetime" "+%s")
                oldtimesecs=0
                oldday=0
                for((i=0; i<$framestotal; i++))
                do
                    timesecs=`echo "scale=0; $startsecs+$i*$frametime/1" | bc`
                    if [ $timesecs -ne $oldtimesecs ]; then             # just calculate frames which differ from each other
                        oldtimesecs=$timesecs
                        year=$(TZ=':CEST' date -d @$timesecs +%Y)
                        month=$(TZ=':CEST' date -d @$timesecs +%m)
                        day=$(TZ=':CEST' date -d @$timesecs +%d)
                        hour=$(TZ=':CEST' date -d @$timesecs +%H)
                        minute=$(TZ=':CEST' date -d @$timesecs +%M)
                        second=$(TZ=':CEST' date -d @$timesecs +%S)
...
#                        hourangle=`echo "scale=2; (($hour+($minute/60))*360/12)/1" | bc`;
                        ...
                        convert $bighand -distort SRT $hourangle $hour_r
